...
const FAQ = () => {
  return (
    <section id="faqContainer" className="bg-slate-50 py-20">
      <div className="container mx-auto grid grid-cols-1 items-center gap-10 px-6 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="join join-vertical w-full overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl shadow-slate-100">
          {faqs.map((faq, index) => (
            <div
              key={faq.question}
              className="collapse collapse-arrow join-item border-b border-slate-200 last:border-b-0"
            >
              <input
                type="radio"
              ...
              />
              <div className="collapse-title text-lg font-extrabold text-slate-950">
                {faq.question}
              </div>
